...
class FeatureDetector:
    ...
    def __kp_des_for_single_img(self, img):
    ...
    def detect_and_compute_orb(self):
        ...
        det_comp = lambda src: self.__kp_des_for_single_img(src)
        for i in range(len(self._srcs)):
            kp, des = self.__kp_des_for_single_img(self._srcs[i])
...
class ObjectLocalizer(FeatureDetector):
    def __init__(self, src1, src2, n_features=10000):
    ...
    def draw_matches(self, scene_corners, matches_mask):
        img_test = cv.polylines(self._srcs[1],[np.int32(scene_corners)], True, 255, 10, cv.LINE_AA)
